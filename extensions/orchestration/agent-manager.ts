...
function finalizeOutputCleanup(record: AgentRecord): void {
...
export class AgentManager {
  ...
  private startAgent(id: string, record: AgentRecord, { pi, ctx, type, prompt, options }: SpawnArgs) {
    ...
    const promise = runAgent(ctx, type, effectivePrompt, {
      ...
      .then(({ responseText, session, aborted, steered }) => {
        ...
        this.finalizeBackground(record, options.isBackground);
        return responseText;
      })
      .catch((err) => {
        console.error("[pi-subagents SPAWN ERROR]", err instanceof Error ? err.stack : err);
        if (record.status !== "stopped") {
          record.status = "error";
        }
  ...
  /** Bookkeeping after a background agent finishes (success or error). */
  private finalizeBackground(record: AgentRecord, isBackground: boolean | undefined): void {
