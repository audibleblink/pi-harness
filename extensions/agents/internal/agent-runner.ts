...
export async function runAgent(
  ...
  if (agentConfig?.memory) {
    ...
    if (hasWriteTools) {
      const memNames = getMemoryToolNames(existingNames);
      if (memNames.length > 0) toolNames = [...toolNames, ...memNames];
      extras.memoryBlock = buildMemoryBlock(agentConfig.name, agentConfig.memory, effectiveCwd);
    } else {
      if (!existingNames.has("read")) {
        const readNames = getReadOnlyMemoryToolNames(existingNames);
        if (readNames.length > 0) toolNames = [...toolNames, ...readNames];
      }
