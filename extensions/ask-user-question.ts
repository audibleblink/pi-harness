...
const OptionSchema = Type.Object({
	...
	description: Type.Optional(
		Type.String({ description: "Help text shown below the label" }),
	),
});
...
const QuestionSchema = Type.Object({
	...
	type: Type.Unsafe<"radio" | "checkbox" | "text">({
		...
		description:
			"Question type: radio (single-select), checkbox (multi-select), or text (free input)",
	}),
	prompt: Type.String({ description: "The question text to display" }),
	label: Type.Optional(
		Type.String({
			description:
				"Short label for tab bar (defaults to Q1, Q2...)",
		}),
	),
	...
	allowOther: Type.Optional(
		Type.Boolean({
			description:
				"Add an 'Other...' option with text input (default: true for radio/checkbox)",
		}),
	),
	...
	placeholder: Type.Optional(
		Type.String({ description: "Placeholder for text inputs" }),
	),
	default: Type.Optional(
		Type.Union([Type.String(), Type.Array(Type.String())], {
			description:
				"Default value(s). String for radio/text, string[] for checkbox",
		}),
	),
});
...
const AskUserQuestionParams = Type.Object({
	title: Type.Optional(
		Type.String({ description: "Form title displayed at the top" }),
	),
	description: Type.Optional(
		Type.String({
			description:
				"Brief context or instructions shown under the title",
		}),
	),
...
export default function askUserQuestion(pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Ask User",
		description:
			`Ask the user one or more questions using an interactive form. Supports three question types:
...
Use this tool when you need user input to proceed — Prefer this over asking plain-text questions in your response.`,
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const result = await ctx.ui.custom<FormResult>(
				(tui, theme, _kb, done) => {
					...
					let otherQuestionId: string | null =
						null;
					let cachedLines: string[] | undefined;

					// Answers store
					const radioAnswers = new Map<
						string,
						{
							value: string;
							label: string;
					...
					const checkAnswers = new Map<
						string,
						Set<string>
					>(); // id -> set of selected values
					const checkCustom = new Map<
						string,
						string
					>(); // id -> custom "other" text
					...
					function saveOtherModeText() {
						...
						const t = editor.getText()
							.trim();
						const oq = questions.find((q) =>
							q.id === otherQuestionId
						);
						if (oq?.type === "radio" && t) {
							radioAnswers.set(
								oq.id,
					...
					function isAnswered(
						...
						if (
							...
							return radioAnswers.has(
								q.id,
							);
						}
						if (q.type === "checkbox") {
							const set = checkAnswers
								.get(q.id);
							const custom =
								checkCustom.get(
									q.id,
					...
					function render(
						...
						if (otherMode) {
							...
							add(theme.fg(
								"dim",
								` ↑↓ navigate • Space toggle • ${nav}Enter ${
									isMulti
										? "next"
										: "submit"
								} • Esc cancel`,
							...
							add(theme.fg(
								"dim",
								` ↑↓ navigate • ${nav}Enter select • Esc cancel`,
							));
						}
						hr();
		...
		renderResult(result, _options, theme, _context) {
			...
			const lines = details.answers.map((a) => {
				const q = details.questions.find((q) =>
					q.id === a.id
				);
				const label = q?.label || a.id;
				...
				return `${theme.fg("success", SYM.check)} ${
					theme.fg("accent", label)
				}: ${a.value || theme.fg("dim", "(empty)")}`;
			});
